...
- Codex 日报停止入库“任务”和“回答简述”，同步移除页面及查询依赖；新增数据库删列迁移，以会话 ID 去重，日报综合总结改为读取本地临时会话材料。
- 调整 Codex 日报会话记录表格：仓库标签改为单独的“状态”列，会话列只显示标题，减少窄屏下横向挤压。
- 调整 Codex 日报会话记录：使用 Ant Design Timeline 改为时间轴展示，只保留时间、会话标题与 Token，移除表格列、分页和表格专用滚动样式。

## 2026-08-29

- 优化首页比例布局：主内容区改为按视口保留 `clamp(32px,4vw,88px)` 左右边距并铺开，顶部三张功能卡与下方分类/预览区共享同一内容边界；分类侧栏改为 `clamp(220px,16vw,300px)`，预览区使用剩余空间，避免大屏定宽居中造成两侧留白过多。

## 2026-08-20 18:10:12 +08:00

- 优化投资页“我的关注”窄面板布局：搜索与新增按钮改为稳定网格，关注项改为顶部标题操作行、双列指标小块和紧凑趋势图，删除按钮回到卡片右上角，减少空白并提升信息层级。

## 2026-08-20 17:54:44 +08:00

- 优化投资页市场信号区域的视觉层级：关注列表、推荐关注、市场信号地图和信号依据统一使用带圆角的主题面板背景，去掉偏灰的半透明底色，并提升说明、来源、时间等辅助文案的对比度，避免浅色模式下文字发白看不清。

## 2026-08-20

- 投资页新增全屏按钮；关注列表改为根据自身面板宽度响应，在窄屏下使用纵向信息布局，并在 1180px 以下释放页面纵向滚动空间。
- 投资页标题、基金和股票图标统一使用红色，侧边栏图标继续沿用主题色。
