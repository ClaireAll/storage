  ...
  return (
    <section className="flex min-h-0 w-full flex-1 flex-col gap-4" data-investment-dashboard>
      ...
      <div className="grid min-h-0 flex-1 grid-cols-[minmax(0,2fr)_minmax(0,3fr)] gap-4 max-[980px]:grid-cols-1">
        <section className="flex min-h-0 flex-col border border-black/8 bg-white/55 p-4 shadow-sm dark:border-white/12 dark:bg-black/10">
          ...
          <div className="mt-4 hidden grid-cols-[minmax(0,1.15fr)_82px_114px_minmax(0,.8fr)_30px] gap-2 border-b border-black/8 pb-2 text-xs text-black/45 min-[720px]:grid dark:border-white/12 dark:text-white/45">
      ...
      <Drawer destroyOnHidden footer={<Button loading={isCreating} onClick={() => void createWatchlistItem()} type="primary">加入关注</Button>} onClose={() => setIsCreateOpen(false)} open={isCreateOpen} title="添加关注" width={400}>
        <div className="flex flex-col gap-4">
          ...
          {searchResults.length ? <div className="divide-y divide-black/8 border-y border-black/8 dark:divide-white/12 dark:border-white/12">{searchResults.map((item) => <button className="flex w-full items-center justify-between gap-3 py-2 text-left" key={`${item.instrumentType}-${item.instrumentCode}`} onClick={() => setForm({ instrumentCode: item.instrumentCode, instrumentName: item.instrumentName, instrumentType: item.instrumentType })} type="button"><span className="truncate">{item.instrumentName}</span><span className="shrink-0 text-xs text-black/45 dark:text-white/45">{item.instrumentCode}</span></button>)}</div> : null}
...
function WatchlistRow({ item, onDragEnd, onDragStart, onDrop, onRemove }: { item: InvestmentWatchlistEntry; onDragEnd: () => void; onDragStart: () => void; onDrop: () => void; onRemove: () => void }) {
  return <div className="flex flex-col gap-2 border-b border-black/7 py-4 last:border-b-0 min-[720px]:grid min-[720px]:grid-cols-[minmax(0,1.15fr)_82px_114px_minmax(0,.8fr)_30px] min-[720px]:items-center min-[720px]:gap-2 dark:border-white/10" draggable onDragEnd={onDragEnd} onDragOver={(event) => event.preventDefault()} onDragStart={onDragStart} onDrop={onDrop}>
