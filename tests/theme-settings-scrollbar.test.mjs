...
test("keeps theme settings scrollable without exposing a native scrollbar", async () => {
  ...
  assert.equal(pageSource.includes("<OverlayScrollArea"), true);
  assert.equal(pageSource.includes("theme-settings-shell h-dvh min-h-0 flex-1"), true);
  assert.equal(pageSource.includes('viewportClassName="flex flex-col overflow-x-hidden!"'), true);
